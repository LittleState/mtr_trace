...
echo -e "——————————————————————————————\n"
for i in {0..8}; do
	mtr -r --n --tcp ${iplise[i]} > ./traceroute_testlog
	grep -q "59\.43\." ./traceroute_testlog
	if [ $? == 0 ];then
		grep -q "202\.97\."  ./traceroute_testlog
		if [ $? == 0 ];then
		...
		grep -q "202\.97\."  ./traceroute_testlog
		if [ $? == 0 ];then
			grep -q "219\.158\." ./traceroute_testlog
			if [ $? == 0 ];then
			...
			echo -e "目标:${iplocal[i]}[${iplise[i]}]\t回程线路:\033[1;34m电信163\033[0m"
				...
				grep -q "218\.105\."  ./traceroute_testlog
				if [ $? == 0 ];then
				echo -e "目标:${iplocal[i]}[${iplise[i]}]\t回程线路:\033[1;35m联通9929\033[0m"
				else
					grep -q "219\.158\."  ./traceroute_testlog
					if [ $? == 0 ];then
						grep -q "219\.158\.113\." ./traceroute_testlog
						if [ $? == 0 ];then
						...
						grep -q "223\.120\."  ./traceroute_testlog
						if [ $? == 0 ];then
						echo -e "目标:${iplocal[i]}[${iplise[i]}]\t回程线路:\033[1;35m移动CMI\033[0m"
						else
							grep -q "221\.183\."  ./traceroute_testlog
							if [ $? == 0 ];then
							echo -e "目标:${iplocal[i]}[${iplise[i]}]\t回程线路:\033[1;35m移动cmi\033[0m"
							else
